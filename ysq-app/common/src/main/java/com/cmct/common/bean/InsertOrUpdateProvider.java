...
//主键冲突则更新
        ...
        for (EntityColumn column : columnList) {
            ...
            if (column.isId() || StringUtil.isNotEmpty(column.getSequenceName()) || column.isIdentity() || column.isUuid()) {
                continue;
            }
            //优先使用传入的属性值,当原属性property!=null时，用原属性
            //其他情况值仍然存在原property中
            sql.append(SqlHelper.getIfNotNull(column, column.getColumn() + " = " + column.getColumnHolder(null, null, ","), isNotEmpty()));
